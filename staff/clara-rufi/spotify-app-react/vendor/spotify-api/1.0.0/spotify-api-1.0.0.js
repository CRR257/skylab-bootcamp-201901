...
const spotifyApi = {
    ...
    searchArtists(query, callback) {
        fetch(`https://api.spotify.com/v1/search?q=${query}&type=artist`, { //el fetch retorna una promise. si fem fetch('https://api.spotify.com/v1/search?q=${query}&type=artist')
            ...
            .then(res => res.json())
            .then(({ artists: { items } }) => callback(undefined, items))  //et ve la id de l'artista
            .catch(callback)
    },
     ...
     * Retrieves albums from artist.
     * 
     * @param {string} albumId - The songs to retrieve albums from.
     * @param {*} callback - callback - The expression to evaluate on response. If error first 
     * argument informs the error message, othwerwise first argument is undefined and second argument provides the matching 
     * results.
     */
    ...
    retrieveTrack(songId, callback) {
        fetch(`https://api.spotify.com/v1/tracks/${songId}`, {
            ...
            .then(res => res.json())
            .then((items) => callback(undefined, items))  //aqui agafem objecte sencer, x aixo no calen {}
            .catch(callback)
        }
}
